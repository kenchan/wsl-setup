...
- The unit is deployed, not generated by `dockerd-rootless-setuptool.sh`, which
  bakes the invoking shell's `PATH` into it and then never rewrites it. The
  managed unit has no `Environment=PATH`, so the daemon follows the systemd user
  manager's `PATH` and stays in step with the shell.
